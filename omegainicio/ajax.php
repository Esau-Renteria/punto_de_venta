...
if(!empty($_POST)){
	...
	if($_POST['action'] == 'infoProducto')
	{
		...
		$query = mysqli_query($conection, "SELECT codproducto,descripcion,existencia,precio FROM producto WHERE codproducto = $producto_id AND estado =1");
...
echo 'error'; //Agregar un r/n no se por que
		exit;
	}
...
if($_POST['action'] == 'delProduct')
	{
		...
		if(empty($_POST['producto_id']) || !is_numeric($_POST['producto_id'])){
			...
			$query_delete = mysqli_query($conection,"UPDATE producto SET estado = 0 WHERE codproducto = $idproducto ");
			mysqli_close($conection);//
			if($query_delete){
				echo 'ok';
 ...
 //Buscarcliente
		...
		//Registrar Cliente - Ventas
			...
			///Agrega producto al detalle temoporal
	if($_POST['action'] == 'addProductoDetalle'){
		if(empty($_POST['producto']) || empty($_POST['cantidad']))
		{
			...
			if($result > 0){
				...
				$detalleTotales ='<tr>
								<td colspan="5" class="textright">S/IVA</td>
