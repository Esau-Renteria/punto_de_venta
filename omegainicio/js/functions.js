            ...
            success: function(response){
                ...
                if(response != 'error'){
                    ...
                    $('.bodyModal').html('<form action="" method="post" name="form_del_product" id="form_del_product" onsubmit="event.preventDefault(); delProduct();">'+
                                         '<h1><i class="fas fa-cubes" style="font-size: 45pt "></i><br> Elimar Producto</h1>'+
...
$('#txt_cod_producto').keyup(function(e){
    ...
    if(producto != '')
    {
     $.ajax({
                ...
                success: function(response)
                {
                   if(response != '\r\nerror') // se le debe poner eso por que quien sabe por que lo agrega
                   {
                    var info = JSON.parse(response);
...
$('#txt_cant_producto').keyup(function(e){
    ...
    //oculta el boton agregar si la cantidad es menor que uno 
            ...
            success: function(response){
                ...
                if(response != 'error'){
                    ...
                    $('.bodyModal').html('<form action="" method="post" name="form_anular_factura" id="form_anular_factura" onsubmit="event.preventDefault(); anularFactura();">'+
                                         '<h1><i class="fas fa-cubes" style="font-size: 45pt "></i><br> EAnular Factura</h1>'+
...
function generarPDF(cliente,factura){
    ...
    //calculamos posicionxy para crear la ventana
    var x = parseInt((window.screen.width/2)-(ancho/2));
    var y = parseInt((window.screen.heigth/2)-(alto/2));

    $url = 'factura/generaFactura.php?cl='+cliente+'&f='+factura;
    window.open($url,"Factura","left="+x+",top="+y+",heigth="+alto+",width="+ancho+",scrollbar=si,location=no,resizable=si,menubar=no");
}
